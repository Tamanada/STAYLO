...
export default function Register() {
  ...
  // Where to send the user after auth completes. Default = /survey for the
  // marketing funnel, but ?next overrides (e.g. /checkout flow that bounced
  // here for sign-in). Same safe-redirect rules as Login.
  const nextPath = safeNext(searchParams.get('next'), '/survey')
  // OAuth providers come back to a fixed URL — we encode the eventual
  // destination so /verify-email can pick it up after the magic link confirms.
  const oauthRedirect =
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // Always route through /verify-email — even for the marketing funnel —
      // so the user knows to confirm their address. The page handles the
      // "already verified, just keep going" case automatically.
      navigate('/verify-email?next=' + encodeURIComponent(nextPath))
    } catch (err) {
    ...
    <div className="max-w-md mx-auto px-4 py-16">
      ...
      <Card className="p-8">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: oauthRedirect } })}
          ...
          <button
            onClick={() => supabase.auth.signInWithOAuth({ provider: 'facebook', options: { redirectTo: oauthRedirect } })}
          ...
          <button
            onClick={() => supabase.auth.signInWithOAuth({ provider: 'twitter', options: { redirectTo: oauthRedirect } })}
        ...
        <div className="flex items-center gap-4 my-6">
          ...
          <span className="text-sm text-gray-400">{t('auth.or_email', 'or register with email')}</span>
          <div className="flex-1 h-px bg-gray-200" />
        </div>
